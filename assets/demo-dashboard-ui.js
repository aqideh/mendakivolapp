(() => {
  ...
  const viewConfig = {
    home: {
      ...
      title: 'Dashboard',
      copy: 'Overview of your volunteer profile, hours, referrals, and points.'
    },
    opportunities: {
      eyebrow: 'Volunteer account',
      title: 'Opportunities',
      copy: 'Review active sign-ups and credited volunteering history.'
    },
    training: {
      eyebrow: 'Volunteer account',
      title: 'Training',
      copy: 'Track training sign-ups and completion status.'
    },
    attendance: {
      eyebrow: 'Volunteer account',
      title: 'Attendance self-reporting',
      copy: 'Submit attendance for confirmed volunteering opportunities and track admin review.'
    },
    referrals: {
      eyebrow: 'Volunteer account',
      title: 'Referrals',
      copy: 'Referral code, referral conversions, and referral-based points.'
    },
    gamification: {
      eyebrow: 'Volunteer account',
      title: 'Gamification',
      copy: 'Points ledger and achievements generated from attendance, training, and referrals.'
    },
    settings: {
      eyebrow: 'Volunteer account',
      title: 'Settings',
      copy: 'Update profile details and account settings.'
    },
    admin: {
      eyebrow: 'Admin workspace',
      title: 'Admin',
      copy: 'Review sign-ups, attendance, training, referrals, and QA checks.'
    }
  };
  ...
  function profile() { return store()?.getProfile?.() || null; }
  function isAdmin() { return Boolean(store()?.isAdmin?.()); }
  function signedIn() { return Boolean(session()?.email); }

  function stat(selector, fallback = '0') {
    return qs(selector)?.textContent?.trim() || fallback;
  }

  function countReferrals() {
    const refs = store()?.getMyReferrals?.() || window.MENDAKIReferrals?.getMyReferrals?.() || [];
    return Array.isArray(refs) ? refs.length : 0;
  }

  function pointsSummary() {
    const pointNode = qs('[data-points-card] .dashboard-stat strong');
    const achievementNode = qsa('[data-points-card] .dashboard-stat strong')[1];
    return {
      points: pointNode?.textContent?.trim() || '0',
      achievements: achievementNode?.textContent?.trim() || '0'
    };
  }

  function ensureRole(card, role) {
    if (card) card.dataset.dashboardCardRole = role;
  }
  ...
  function ensureHomeShell() {
    ...
    if (!home && layout) {
      ...
      if (profileCard) profileCard.insertAdjacentElement('afterend', home);
    ...
    home.innerHTML = `
      <div class="dashboard-hub-header">
        <div>
          <p class="eyebrow dark">Volunteer modules</p>
          <h2>Choose a dashboard area</h2>
          <p>Use these demo-friendly sections to present volunteering history, attendance, referrals, and gamification.</p>
        </div>
      </div>
  ...
  function updateVisibility(view = 'home') {
    const page = qs('#page-dashboard');
    const layout = qs('.dashboard-layout');
    if (!page || !layout) return;
    const next = viewConfig[view] ? view : 'home';
    if (next === 'admin' && !isAdmin()) return updateVisibility('home');
    applyRoles();
    renderTiles();
    ...
    qsa('[data-dashboard-card-role]').forEach(card => {
      const role = card.dataset.dashboardCardRole;
      card.style.display = inHome ? 'none' : (role === next ? 'grid' : 'none');
    });
    const profileCard = qs('.dashboard-profile-card');
    ...
    qsa('[data-dashboard-view-target]').forEach(button => {
      button.classList.toggle('active', button.dataset.dashboardViewTarget === next);
    });
  }
  ...
  function improveProfileSummary() {
    ...
    const points = pointsSummary();
    const items = [
      ['Verified hours', stat('[data-stat-hours]', '0')],
      ['Points', points.points],
      ['Referrals', String(countReferrals())],
  ...
  function install() {
    ...
    updateVisibility(qs('#page-dashboard')?.dataset.demoDashboardView || 'home');
  }
