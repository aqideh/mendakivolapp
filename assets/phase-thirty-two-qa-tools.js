(() => {
  if (window.__phaseThirtyTwoQaToolsInstalled) return;
  window.__phaseThirtyTwoQaToolsInstalled = true;
  ...
  function result(status, label, detail = '') {
    state32.results.push({ status, label, detail });
    render();
  }

  function counts() {
    return state32.results.reduce((acc, item) => {
  ...
  function ensureCard() {
    ...
    let card = document.querySelector('[data-phase32-qa-card]');
    if (card) return card;
    card = document.createElement('section');
    card.className = 'dashboard-card phase32-qa-card';
    card.dataset.phase32QaCard = 'true';
    ...
    card.innerHTML = `
      <div class="section-header">
        ...
        <div class="phase32-qa-actions">
          <button class="button button-primary" type="button" data-phase32-run ${state32.running ? 'disabled' : ''}>${state32.running ? 'Running...' : 'Run checks'}</button>
          <button class="button dashboard-secondary" type="button" data-phase32-clear ${state32.running ? 'disabled' : ''}>Clear</button>
        </div>
      </div>
      ...
      <p class="dashboard-muted">${state32.lastRunAt ? `Last run: ${escapeHtml(state32.lastRunAt)}` : 'Not run yet.'}</p>
      <div class="phase32-qa-result-list">
        ${state32.results.length ? state32.results.map(renderResult).join('') : '<div class="admin-content-item"><span>No checks run yet.</span></div>'}
      </div>
    `;
  }
  ...
  async function runChecks() {
    if (state32.running) return;
    state32.running = true;
    state32.results = [];
    state32.lastRunAt = new Date().toLocaleString('en-SG');
    render();
    try {
      await checkAuth();
      ...
      state32.running = false;
      render();
      if (window.MENDAKIPhase31AdminUX?.install) window.MENDAKIPhase31AdminUX.install();
    }
  }

  function bind() {
    if (window.__phaseThirtyTwoQaToolsBound) return;
    window.__phaseThirtyTwoQaToolsBound = true;
    document.addEventListener('click', event => {
      if (event.target.closest('[data-phase32-run]')) {
        event.preventDefault();
        runChecks();
        return;
      }
      if (event.target.closest('[data-phase32-clear]')) {
        state32.results = [];
        state32.lastRunAt = '';
        render();
      }
  ...
  window.MENDAKIPhase32QA = { install, runChecks };
