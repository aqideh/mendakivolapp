(() => {
  ...
  function captureReferralFromUrl() {
    ...
    const code = normaliseCode(params.get('ref') || params.get('referral') || hashParams.get('ref') || hashParams.get('referral') || '');
    if (code) localStorage.setItem(REF_STORAGE_KEY, code);
    return code;
  }

  function pendingReferralCode() { return normaliseCode(localStorage.getItem(REF_STORAGE_KEY) || ''); }
  ...
  async function acceptPendingReferral() {
    ...
    if (data?.ok || ['already_referred', 'self_referral', 'invalid_code'].includes(data?.reason)) localStorage.removeItem(REF_STORAGE_KEY);
    return data;
  }
  ...
  function statusLabel(status) {
    return ({ accepted: 'Accepted', converted: 'Converted', cancelled: 'Cancelled', duplicate: 'Duplicate' })[status] || status || 'Unknown';
  }

  function statusClass(status) {
    const value = String(status || '').toLowerCase();
    return ['accepted', 'converted', 'cancelled', 'duplicate'].includes(value) ? value : 'unknown';
  }

  function statusPill(status) {
    return `<span class="referral-status-pill ${escapeHtml(statusClass(status))}">${escapeHtml(statusLabel(status))}</span>`;
  }
  ...
  function renderVolunteerCardBody() {
    if (!signedIn()) {
      ...
      return `<div class="section-header"><div><p class="eyebrow dark">Invite friends</p><h2>Referral invite</h2><p class="dashboard-muted">${pending ? `Referral code ${escapeHtml(pending)} is saved. Sign in or create an account to accept it.` : 'Sign in to get your referral link and invite friends to volunteer with you.'}</p></div></div><button class="button button-primary" type="button" data-auth-open data-auth-entry>Sign in to use referrals</button>`;
    }
    const link = referralLink();
    return `
      <div class="section-header"><div><p class="eyebrow dark">Invite friends</p><h2>Referral invite</h2><p class="dashboard-muted">Share your referral link with friends who want to volunteer with MENDAKI.</p></div><button class="text-button" type="button" data-referral-refresh>${loading ? 'Loading...' : 'Refresh'}</button></div>
      <div class="referral-code-panel"><span>Your code</span><strong>${escapeHtml(currentCode || 'Generating...')}</strong></div>
      <label class="referral-link-field"><span class="dashboard-muted">Referral link</span><input readonly value="${escapeHtml(link || '')}" data-referral-link-input></label>
      <div class="dashboard-actions"><button class="button button-primary" type="button" data-referral-copy ${link ? '' : 'disabled'}>Copy link</button><button class="button dashboard-secondary" type="button" data-referral-share ${link ? '' : 'disabled'}>Share</button></div>
      <div class="referral-list"><h3>Your referrals</h3>${myReferrals.length ? myReferrals.map(renderMyReferral).join('') : '<p class="dashboard-muted">No accepted referrals yet.</p>'}</div>
  ...
  function renderMyReferral(item) {
    const date = item.accepted_at ? displayDate(item.accepted_at) : '';
    return `<div class="admin-content-item referral-row"><span><strong>${escapeHtml(item.referred_name || item.referred_email || 'Referred volunteer')}</strong><span>${date ? `Accepted ${escapeHtml(date)}` : 'Referral recorded'}</span></span>${statusPill(item.status)}</div>`;
  }

  function renderAdminCardBody() {
    return `<div class="section-header"><div><p class="eyebrow dark">Admin</p><h2>Referral tracking</h2><p class="dashboard-muted">View accepted referrals for pilot operations and future points attribution.</p></div><button class="text-button" type="button" data-referral-refresh>${loading ? 'Loading...' : 'Refresh'}</button></div><div class="admin-content-list page-list">${adminReferrals.length ? adminReferrals.map(renderAdminReferral).join('') : '<div class="admin-content-item"><span>No referrals recorded yet.</span></div>'}</div>`;
  }
  ...
  function renderAdminReferral(item) {
    ...
    const date = item.accepted_at ? displayDate(item.accepted_at) : '';
    const meta = `${item.referral_code || ''}${date ? ` · ${date}` : ''}`;
    return `<div class="admin-content-item referral-row"><span><strong>${escapeHtml(referrer)} → ${escapeHtml(referred)}</strong><span>${escapeHtml(meta)}</span></span>${statusPill(item.status)}</div>`;
  }
  ...
  async function shareReferralLink() {
    const link = referralLink();
    if (!link) return;
    if (navigator.share) await navigator.share({ title: 'Volunteer with MENDAKI', text: 'Join me on the MENDAKI Volunteer Hub.', url: link }).catch(() => null);
  ...
  function bind() {
    ...
    document.addEventListener('click', event => {
      if (event.target.closest('[data-referral-copy]')) { copyReferralLink(); return; }
      if (event.target.closest('[data-referral-share]')) { shareReferralLink(); return; }
      if (event.target.closest('[data-referral-refresh]')) sync();
    }, true);
  }

  window.MENDAKIReferrals = { sync, render, captureReferralFromUrl, getMyReferrals: () => myReferrals.slice(), getAdminReferrals: () => adminReferrals.slice() };

  document.addEventListener('DOMContentLoaded', () => { bind(); exposeStoreHelpers(); window.setTimeout(sync, 900); });
