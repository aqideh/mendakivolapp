(() => {
  if (window.__phaseThirtyOneAdminUxInstalled) return;
  window.__phaseThirtyOneAdminUxInstalled = true;
  ...
  function ensureHub() {
    ...
    let hub = document.querySelector('[data-phase31-admin-hub]');
    if (hub) return hub;
    hub = document.createElement('section');
    hub.className = 'dashboard-card admin-ux-hub';
    hub.dataset.phase31AdminHub = 'true';
  ...
  function hubMarkup() {
    return `
      ...
      <div class="admin-ux-tabs" role="tablist" aria-label="Admin work areas">
        ${AREAS.map(([key, label]) => `<button class="admin-ux-tab ${key === state31.activeArea ? 'active' : ''}" type="button" data-admin-ux-area="${key}">${escapeHtml(label)}</button>`).join('')}
      </div>
      <div class="admin-ux-summary-grid" data-admin-ux-summary></div>
      <div class="admin-ux-toolbar">
        <label>Search<input data-admin-ux-search placeholder="Filter visible admin cards" value="${escapeHtml(state31.search)}"></label>
        <label>Status<select data-admin-ux-status>
          ${['', 'pending_review', 'registered', 'waitlisted', 'submitted', 'checked_in', 'completed', 'verified', 'Open', 'Closed'].map(value => `<option value="${escapeHtml(value)}" ${value === state31.status ? 'selected' : ''}>${escapeHtml(value || 'Any status')}</option>`).join('')}
        </select></label>
  ...
  function classifyCards() {
    document.querySelectorAll('.dashboard-card').forEach(card => {
      if (card.dataset.phase31AdminHub === 'true') return;
  ...
  function textMatches(card) {
    const term = state31.search.trim().toLowerCase();
    const status = state31.status.trim().toLowerCase();
  ...
  function applyAreaVisibility() {
    ...
    document.querySelectorAll('[data-admin-ux-area]').forEach(card => {
      ...
      const visibleByArea = state31.activeArea === 'home' || state31.activeArea === area;
    ...
    document.querySelectorAll('[data-admin-ux-area].admin-ux-tab').forEach(button => {
      button.classList.toggle('active', button.dataset.adminUxArea === state31.activeArea);
    });
    renderSummary();
  }
  ...
  function countStatus(items, statuses) {
    ...
    return items.filter(item => set.has(String(item.status || ''))).length;
  }
  ...
  function renderSummary() {
    ...
    const tiles = [
      ...
      ['Training sessions', state31.trainingRows.length || trainings().length]
    ];
    host.innerHTML = tiles.map(([label, value]) => `
      <div class="admin-ux-summary-tile"><strong>${escapeHtml(value)}</strong><span>${escapeHtml(label)}</span></div>
    `).join('');
  }
  ...
  function parentTrainingRows() {
    const seen = new Set();
    return state31.trainingRows.filter(row => {
  ...
  async function fetchTrainingRows() {
    ...
    if (error) {
      console.warn('Could not load training rows for Phase 31 admin UX.', error);
      return [];
    }
    state31.trainingRows = Array.isArray(data) ? data : [];
    return state31.trainingRows;
  }

  function ensureTrainingManager() {
    if (!isAdmin()) return null;
    let card = document.querySelector('[data-phase31-training-manager]');
    if (card) return card;
    card = document.createElement('section');
    card.className = 'dashboard-card phase31-training-card';
    card.dataset.phase31TrainingManager = 'true';
  ...
  function trainingManagerMarkup() {
    const selected = state31.selectedTrainingId || parentTrainingRows()[0]?.id || '';
    state31.selectedTrainingId = selected;
    const parent = state31.trainingRows.find(row => row.id === selected) || {};
    const editing = state31.trainingRows.find(row => row.id === state31.editingSessionId) || null;
    return `
      <div class="section-header">
        <div>
          <p class="eyebrow dark">Admin · Training</p>
          <h2>Training session management</h2>
          <p class="dashboard-muted">Manage parent training rows and child session instances created by Phase 30.</p>
        </div>
        <button class="text-button" type="button" data-phase31-training-refresh>${state31.busy ? 'Loading...' : 'Refresh'}</button>
      </div>
      ...
      <div class="phase31-training-status" data-phase31-training-status></div>
    `;
  }
  ...
  function renderTrainingParent(row) {
    ...
    return `
      <article class="phase31-training-parent ${state31.selectedTrainingId === row.id ? 'active' : ''}" data-phase31-parent-training="${escapeHtml(row.id)}">
        <header>
          <div>
        ...
        <div class="phase31-training-actions">
          <button class="button dashboard-secondary" type="button" data-phase31-select-training="${escapeHtml(row.id)}">Manage sessions</button>
          <button class="button dashboard-secondary" type="button" data-phase31-new-training-session="${escapeHtml(row.id)}">New session</button>
        </div>
    ...
    return `
      <div class="phase31-training-session-item">
        ...
        <div class="phase31-training-session-actions">
          <button class="text-button" type="button" data-phase31-edit-training-session="${escapeHtml(session.id)}" data-parent-training="${escapeHtml(parent.id)}">Edit</button>
          ${session.id !== parent.id ? `<button class="text-button" type="button" data-phase31-delete-training-session="${escapeHtml(session.id)}">Delete</button>` : ''}
        </div>
      </div>
    `;
  }
  ...
  function renderTrainingSessionForm(parent, session) {
    ...
    return `
      <form class="phase31-training-form" data-phase31-training-session-form>
        ...
        <div class="phase31-training-actions">
          ...
          <button class="button dashboard-secondary" type="button" data-phase31-new-training-session="${escapeHtml(parent.id || '')}">New session</button>
        </div>
      </form>
    `;
  }
  ...
  function setTrainingStatus(text) {
    const node = document.querySelector('[data-phase31-training-status]');
  ...
  function sessionRowFromForm(form) {
    ...
    const parent = state31.trainingRows.find(row => row.id === parentId) || {};
  ...
  async function saveTrainingSession(form) {
    ...
    const { error } = await supabase.from(TRAINING_TABLE).upsert(row, { onConflict: 'id' });
    state31.busy = false;
    if (error) {
      setTrainingStatus(`Could not save: ${error.message}`);
      return;
    }
    state31.editingSessionId = row.id;
    await fetchTrainingRows();
    await store()?.syncPhaseThirtyTrainingSessions?.().catch?.(() => null);
  ...
  async function deleteTrainingSession(id) {
    ...
    const { error } = await supabase.from(TRAINING_TABLE).delete().eq('id', id);
    if (error) {
      window.alert(`Could not delete training session: ${error.message}`);
      return;
    }
    state31.editingSessionId = '';
    await fetchTrainingRows();
    await store()?.syncPhaseThirtyTrainingSessions?.().catch?.(() => null);
    renderTrainingManager();
  }
  ...
  async function renderTrainingManager() {
    ...
    if (!card) return;
    if (!state31.trainingRows.length) await fetchTrainingRows();
  ...
  function bind() {
    if (window.__phaseThirtyOneAdminUxBound) return;
    window.__phaseThirtyOneAdminUxBound = true;
    document.addEventListener('click', async event => {
      const area = event.target.closest('.admin-ux-tab[data-admin-ux-area]');
      if (area) {
        state31.activeArea = area.dataset.adminUxArea || 'home';
        applyAreaVisibility();
        return;
      }
      ...
      const selectTraining = event.target.closest('[data-phase31-select-training]');
      if (selectTraining) {
        state31.selectedTrainingId = selectTraining.dataset.phase31SelectTraining || '';
        state31.editingSessionId = '';
        renderTrainingManager();
        return;
      }
      const newSession = event.target.closest('[data-phase31-new-training-session]');
      if (newSession) {
        state31.selectedTrainingId = newSession.dataset.phase31NewTrainingSession || state31.selectedTrainingId;
        state31.editingSessionId = '';
        renderTrainingManager();
        return;
      }
      const editSession = event.target.closest('[data-phase31-edit-training-session]');
      if (editSession) {
        state31.selectedTrainingId = editSession.dataset.parentTraining || state31.selectedTrainingId;
        state31.editingSessionId = editSession.dataset.phase31EditTrainingSession || '';
        renderTrainingManager();
        return;
      }
      const deleteSession = event.target.closest('[data-phase31-delete-training-session]');
      if (deleteSession) {
        await deleteTrainingSession(deleteSession.dataset.phase31DeleteTrainingSession);
        return;
      }
      if (event.target.closest('[data-phase31-training-refresh]')) {
        await fetchTrainingRows();
        renderTrainingManager();
      }
    }, true);
    ...
    document.addEventListener('change', event => {
      ...
      if (status) {
        state31.status = status.value || '';
        applyAreaVisibility();
      }
    }, true);

    document.addEventListener('submit', event => {
      const form = event.target.closest('[data-phase31-training-session-form]');
      if (!form) return;
  ...
  window.MENDAKIPhase31AdminUX = { install, renderTrainingManager, fetchTrainingRows };
