(() => {
  ...
  function setBusy(button, busy, label = 'Saving...') {
    if (!button) return;
    if (busy) {
      button.dataset.volunteerActionOriginalText = button.textContent || '';
      button.disabled = true;
      button.textContent = label;
      return;
    }
    button.disabled = false;
    if (button.dataset.volunteerActionOriginalText) button.textContent = button.dataset.volunteerActionOriginalText;
    delete button.dataset.volunteerActionOriginalText;
  }
  ...
  window.MENDAKIVolunteerActions = Object.freeze({ refreshAll });
})();
